...
public class HashvsArray extends AppCompatActivity {

        private Map<String, Integer> performanceMap = new HashMap<>();
//    private Map<String, Integer> performanceMap = new ArrayMap<>();
...
//        request.setMap_structure( "arrayMap" );
        ...
        if (request != null) {
            new APIHelper().postData(request, new StringCallBack() {
                ...
                public void execute(String str) {
                    Toast.makeText(HashvsArray.this, "aaaaa", Toast.LENGTH_SHORT).show();
                }
            });
        }
    ...
    private void performRandomQuery() {
        ...
        for (int i = 0; i < dataSize; i++) {
            ...
            int value = performanceMap.get("Key" + randomIndex);
            arrValue[i] = value;
        }
        long endTime = Debug.threadCpuTimeNanos();
